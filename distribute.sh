...
function push_arm() {
  ...
  export OLD_CMAKECROSS=$CMAKECROSS
  ...
  CMAKECROSS="cmake"
  if [ $DEBUG -eq 1 ]; then
    CMAKECROSS="${CMAKECROSS} -DCMAKE_BUILD_TYPE=Debug"
  else
    CMAKECROSS="${CMAKECROSS} -DCMAKE_BUILD_TYPE=Release"
  fi
  CMAKECROSS="${CMAKECROSS} -DENABLE_VISIBILITY=1 -DIOS_ARCH=${ARCH} -IOS_PLATFORM=${PLATFORM}"
  CMAKECROSS="${CMAKECROSS} -DCMAKE_TOOLCHAIN_FILE=$CMAKE_TOOLCHAIN_FILE -DCMAKE_INSTALL_PREFIX:PATH=$STAGE_PATH"
  CMAKECROSS="${CMAKECROSS} -DIOS_SYSROOT=$SYSROOT -DIOS_DEPLOYMENT_TARGET=${IOS_MIN_SDK_VERSION} -DQT_PATH=$QT_PATH"
  CFLAGS="${CFLAGS} $VERSION_MIN"

  if false; then
    CMAKECROSS="${CMAKECROSS} -DENABLE_BITCODE=1"
    CFLAGS="${CFLAGS} -fembed-bitcode"
  else
    CMAKECROSS="${CMAKECROSS} -DENABLE_BITCODE=0"
    CFLAGS="${CFLAGS} -headerpad_max_install_names"
  fi

  if true; then
    CMAKECROSS="${CMAKECROSS} -DENABLE_ARC=0"
    CFLAGS="${CFLAGS} -fno-objc-arc"
  else
    CMAKECROSS="${CMAKECROSS} -DENABLE_ARC=1"
  ...
  CMAKECROSS="${CMAKECROSS} -DCMAKE_C_COMPILER=${XCODE_DEVELOPER}/usr/bin/gcc"
  CMAKECROSS="${CMAKECROSS} -DCMAKE_CXX_COMPILER=${XCODE_DEVELOPER}/usr/bin/g++"

  export CMAKECROSS=$CMAKECROSS
...
function pop_arm() {
  ...
  export CMAKECROSS=$OLD_CMAKECROSS
}
